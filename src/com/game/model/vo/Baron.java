...
public class Baron extends Character5G {
	
	private Scanner scan = new Scanner(System.in);

	public Baron() {
		super("내셔 남작", 800, 200000, 0, "초보내셔 남작", "내셔 남작의 콧물", "", "", "치킨킥", "치킨 펀치");

	}

	public void cry() {

		System.out.println("내셔 남작가 위용을 뿜으며 등장합니다");

	}

	public void peck(User user) {
		System.out.println("내셔 남작가 할퀴었습니다.");
	...
	public void lossHp(User user) {
		...
		user.passive();
			...
			while (true) {
				...
				switch (select) {
				case 1:
					System.out.println("내셔 남작이 비명을 지르며 hp가" + user.attack + "만큼 감소합니다");
					super.setHp(super.getHp() - user.attack);

					break;
				case 2:
					System.out.println("내셔 남작이 비명을 지르며 hp가" +user.spskill1dmg + "만큼 감소합니다");
					super.setHp(super.getHp() - user.spSkill1());

					break;
				case 3:
					super.setHp(super.getHp() - user.spSkill2());
					System.out.println("내셔 남작이 비명을 지르며 hp가" + user.spskill2dmg + "만큼 감소합니다");
					break;

				case 4:
					super.setHp(super.getHp() - user.spSkill3());
					System.out.println("내셔 남작이 비명을 지르며 hp가" + user.spskill3dmg + "만큼 감소합니다");

					break;
				case 5:
					super.setHp(super.getHp() - user.finaleSkill());
					System.out.println("내셔 남작이 비명을 지르며 hp가" + user.fskilldmg + "만큼 감소합니다");
				...
				if (super.getHp() <= 0)	{
					System.out.println("내셔 남작가 쓰러집니다 ");
					User.coin+=1000;
					break;
					
				}
				System.out.println("내셔 남작의 남은 체력 :" + hp);
